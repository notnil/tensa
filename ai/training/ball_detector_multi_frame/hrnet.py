...
class HRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # # Improved traceability with TorchScript by refactoring to a constant-sized output (i.e. tuple instead of dict)
        assert self._out_scales == [
            0
        ], "revisit the below optimization if this parameter changes"
        scale = 0
        x = y_list[scale]
        for i in range(self.num_deconvs):
            x = self.deconv_layers[i][scale](x)
        y_out = self.final_layers[scale](x)
        # y_out = {}
        # for scale in self._out_scales:
        #     x = y_list[scale]
        #     for i in range(self.num_deconvs):
